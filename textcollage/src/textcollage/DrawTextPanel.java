...
//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
		for (int n = 0; n < theString.size(); n++) // this is the answer for the third question, every string will
													// rotate adding 1 to each string
	...
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			...
			// implemented.");
			File textFile = fileChooser.getOutputFile(this, "Select Text File Name", "imgFile.txt");
			if (textFile == null)
				return;
			try {
				PrintWriter print = new PrintWriter(textFile);
				Color background = canvas.getBackground();
				print.println(background.getRGB());
				for (DrawTextItem m : theString) {
					print.println(m.getString());
					print.println(m.getTextColor().getRGB());
					print.println(m.getX());
					print.println(m.getY());
					print.println(m.getRotationAngle());
				}
				print.flush();
				print.close();
				canvas.repaint();
			...
			canvas.repaint();
			try {
				Scanner scanner = new Scanner(textFile); // to read from the text file
				Color background = Color.decode(scanner.next()); // decode to be able to change the int value to color
				canvas.setBackground(background); // setting the background color to the previous value
				scanner.nextLine();
				while (scanner.hasNext()) {
					String text = scanner.nextLine();
					DrawTextItem s = new DrawTextItem(text);
					Color textcol = Color.decode(scanner.next()); // get the color of the text
					s.setTextColor(textcol); // set the color to be the color of the current string
					scanner.nextLine();
					s.setX(Integer.parseInt(scanner.next())); // set the x-coordinate of the string
					scanner.nextLine();
					s.setY(Integer.parseInt(scanner.next())); // set the y-coordinate of the string
					scanner.nextLine();
					s.setRotationAngle(Double.parseDouble(scanner.next())); // get the value of the rotation
					scanner.nextLine();
					theString.add(s); // add the string to the ArrayList theString
				}
			...
			DrawTextItem temp = theString.get(theString.size() - 1);
			theString.remove(temp); // Remove the ONLY string from the canvas.
			if (theString.size() == 0)
				undoMenuItem.setEnabled(false);
			canvas.repaint();
